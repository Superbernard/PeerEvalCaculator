...
#   misspelling and some provide nickname etc. Text match methed need to be used to match those
...
data_ognz <- function(rawdata){
  ...
  emptyCol = seq(10,28,2)  #column idx of empty columns
  whole_data <- rawdata[,-c(2,3,5,6,7,8,29,30,31,32,33, emptyCol)] 
...
score_compute <- function(name_score_raw,roster,save_dir,output_filename){
  ...
  for (i in 1:num_sections ){
    ...
    More_than_3_evaluator <- rep(0,dim(num_score_by_sec[[i]])[1] )
    if ( length(idx_abnorm[[i]]) > 0 )  More_than_3_evaluator[idx_abnorm[[i]]] <-1
    
    section_info <- rep(sections_level[i],dim(num_score_by_sec[[i]])[1] ) 
    num_score_by_sec[[i]] <- cbind(num_score_by_sec[[i]],section_info,More_than_3_evaluator)
    # add a new variable with info of if each name is matched more than 3 times 
  }
  ...
  colnames( result_check_wholeClass ) <- c("Name", "Number of evaluator", "Section","More than 3 evaluator")
...
mainFunc <- function(rawdata,roster,save_dir,output_filename,Allsec = T){
  ...
  if (Allsec == F){  # if calculate score for single section
    section_spcy <- paste("BIOL-1121-",svalue(txt_sec_num),sep = "")   #get section level
    Survey_Data <- Survey_Data[Survey_Data$section == section_spcy, ]  #survey data of specified section
    roster <- roster[roster$Section == section_spcy, ]  #survey data of specified section
  }
  
  roster$Section <- factor(roster$Section)
  Survey_Data$section <- factor(Survey_Data$section)
  roster$Student <- as.character(roster$Student)
...
############Build GUI################### 
w = gwindow("PeerScore Calaulator 2017SP v2.2",width = 300 , height = 300, visible = F) # creat a window
...
Sec_label <- glabel(
  "Alll sections or single section ?",
  container = g1
)

SecSel <- gdroplist(c("All Sections", "Single Section"), count = T, cont = g1)  #dropdown list for mixIT

addSpring(g1)

g2 <- ggroup(container = gg,spacing = 10, horizontal = T) # secondary wedget container

lbl_sec_num <- glabel(
  "Enter 3-digit Section number: ",
  container = g2
)
addSpring(g1)
txt_sec_num <- gedit("001", cont = g2)
  ...
  handler   = function(h, ...)
  {
    gfile(
      ...
      handler = function(h, ...)
      {
        tryCatch(
          {
            data_frame_name <- make.names("Survey_Data")
  ...
  handler   = function(h, ...)
  {
    gfile(
      ...
      handler = function(h, ...)
      {
        tryCatch(
          {
            data_frame_name <- make.names("Roster")
    ...
    tryCatch(
      {
        ...
        SecSel_value <- svalue(SecSel) #Yes or No for All sections
        
        if (SecSel_value == "All Sections"){
          status_bar <- gstatusbar("All Sections", container = w)
          Allsec = T
        }else{
          status_bar <- gstatusbar("Single Section", container = w)
          Allsec = F
        }
        
        assign("Allsec", Allsec, envir = globalenv())
        
        output<- mainFunc(Survey_Data,Roster,svalue(save_dir),data_frame_name, Allsec)
