...
if(length(pack_install_idx) > 0){  
  install.packages(package_needed[pack_install_idx])      # Install the pakage(s) needed but not installed
}
...
data_ognz <- function(rawdata){
  ...
  whole_data <- rawdata[,-c(2,3,5,6,7,8,9,10,31,32,33, emptyCol)]      #delete unnecessary info from the dataset 
  
  col_teamate <- paste(c("Teamate","Score","Reason"), rep(1:3, each = 3))      # create col names strings
  colnames(whole_data) <- c(colnames(whole_data)[1:2], col_teamate, "Self")       # assign col names for the whole dataset
  ...
  if ( length(idx_na_score)>0 )  name_score_raw <- name_score_raw[-idx_na_score,]      # remove rows with NA socres 
  
  idx_na_name <- which(is.na(name_score_raw$team_name_raw))      # index of nas in name
  
  if ( length(idx_na_name)>0 )  name_score_raw <- name_score_raw[-idx_na_name,]     # remove rows with na socres 
  
  name_input_total <- as.character(name_score_raw$team_name_raw)       # character input of name input
  ...
  for ( i in 1:length(pattern_to_check) ){
    ...
    if ( length(idx_pattern_reco)>0 ) name_score_raw <- name_score_raw[-idx_pattern_reco,]      # remove names with incorrect pattern
  }
  
  whole_data$section <- as.factor(whole_data$section)      # convert section back to factor
...
ClosestMatch = function(string, stringVector){  
  # function to find best match based on Levenshtein distance
...
score_compute <- function(name_score_raw,roster,save_dir,output_filename){
  ...
  name_standard_by_section <- list()       # create empty list to hold different name subset, (of different section)
  ...
  for ( i in 1:num_sections ){
    ...
    for ( j in 1:num_names_by_section ){
      ...
      score_match[j] <- mean(data_by_section[[i]][data_by_section[[i]]$name_match == name_output[j],
                                                  "team_score_raw"])     # peer scores computed as mean of input scores from all evaluators
  ...
  colnames( caculated_score_wholeClass ) <- c("Name", "Section", "PeerScore")       # respecify col names
